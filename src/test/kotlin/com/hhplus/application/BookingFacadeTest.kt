...
internal class BookingFacadeTest {
    ...
    @BeforeAll
    internal fun init() {
        user = User(name = "test", balance = 10000)
        userRepository.save(user)
        bookingRepository.save(
            Booking(seatId = 1, bookingDate = dateTimeParser.stringToDateTime("2023-12-13 15:30"), status = BookingStatusCode.AVAILABLE,
            price = 3000)
        )
        bookingRepository.save(
            Booking(seatId = 1, bookingDate = dateTimeParser.stringToDateTime("2023-12-13 16:30"), status = BookingStatusCode.AVAILABLE,
                price = 3000)
        )
        bookingFacade.reserveSeat(seatId = 1, bookingDate = "2023-12-13 16:30", uuid = user.uuid!!)
        bookingRepository.save(
            Booking(seatId = 1, bookingDate = dateTimeParser.stringToDateTime("2023-12-13 17:30"),
            status = BookingStatusCode.CONFIRMED, price = 5000)
        )
        bookingRepository.save(
            Booking(seatId = 2, bookingDate = dateTimeParser.stringToDateTime("2023-12-13 15:30"), status = BookingStatusCode.AVAILABLE,
                price = 3000)
        )
    }
    ...
    fun `예약 시 유효한 쿠폰의 정보 아닐 시 오류를 발생`() {
        assertThrows(FailedReserveException::class.java) {
            user.uuid?.let { bookingFacade.reserveSeat(seatId = 1, bookingDate = "2023-12-13 19:30", uuid = it) }
        }
    }
    ...
    fun `예약 시 이미 예약이 되어 있다면 오류를 발생`() {
        assertThrows(FailedReserveException::class.java) {
            bookingFacade.reserveSeat(seatId = 1, bookingDate = "2023-12-13 15:30", uuid = user.uuid!!)
            bookingFacade.reserveSeat(seatId = 1, bookingDate = "2023-12-13 15:30", uuid = user.uuid!!)
        }
    }
    ...
    fun `예약이 가능하면 예약을 성공한다`() {
        ...
        assertThat(bookingRepository.findBySeatIdAndBookingDateAndStatusAndUserUuid(seatId = 1,
            bookingDate = dateTimeParser.stringToDateTime("2023-12-13 15:30"), availableCode = BookingStatusCode.RESERVED.code, userUuid = 1).size).isNotNull()
    }

    @Test
    @Order(1)
    fun `예약 가능한 날짜 조회시 조회 된다`() {
        assertThat(bookingFacade.findDatesAvailable(seatId = 1).size).isNotNull()
    }
    ...
    fun `동시에 10개 예약 요청이 오면 하나의 요청 빼곤 실패해야 한다`() {
        ...
        repeat(10) {
            executor.submit{
                try {
                    bookingFacade.reserveSeat(seatId = 2, bookingDate = "2023-12-13 15:30", uuid = user.uuid!!)
                } catch (_: Exception) {
