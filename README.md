...
## 항해 플러스 3기 프로젝트 기록소
+ 할 일들 List up
  + 토큰 만료된 후 대기열 토큰 큐 poll 후 유효 토큰으로 이동하는 로직 구현
  + 예약정보 레디스 아무리 생각해도 필요 없다. 분산락 + 레디스 삭제하고 db + 낙관적 락으로 재구현 (비관적 락도 아니고 낙관적 락을 사용할 수 있는데 분산락을 활용하는 건 가치가 없다.)
  + 스케줄러로 1분마다 돌면서 5분 넘어간 예약 이력 확인 후 삭제, 예약이 만료 되었을 경우
...
+ 주요 고민들
  ...
  + 기존 lettuce 라이브러리 대신 redisson 사용
    + 기존 스핀락 방식 대신 pub-sub 형태로 분산락을 구현하였기에 성능상 이점 때문에 채택하였으나.. 오버엔지니어링이란 사실을 알고 그냥 lettuce 쓸 걸 그랬음
  + 대기열 설계
